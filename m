...
Received: from lists.sourceforge.net (lists.sourceforge.net [216.105.38.7])
	by mail.lfdr.de (Postfix) with ESMTPS id CF6BFEB467
	for <lists+linux-f2fs-devel@lfdr.de>; Thu, 31 Oct 2019 17:01:29 +0100 (CET)
Received: from [127.0.0.1] (helo=sfs-ml-2.v29.lw.sourceforge.com)
	by sfs-ml-2.v29.lw.sourceforge.com with esmtp (Exim 4.90_1)
	(envelope-from <linux-f2fs-devel-bounces@lists.sourceforge.net>)
	id 1iQCt4-0007lT-Am; Thu, 31 Oct 2019 16:01:26 +0000
Received: from [172.30.20.202] (helo=mx.sourceforge.net)
 by sfs-ml-2.v29.lw.sourceforge.com with esmtps
 (TLSv1.2:ECDHE-RSA-AES256-GCM-SHA384:256) (Exim 4.90_1)
 (envelope-from <jaegeuk@kernel.org>) id 1iQCt2-0007lM-Qb
 for linux-f2fs-devel@lists.sourceforge.net; Thu, 31 Oct 2019 16:01:24 +0000
DKIM-Signature: v=1; a=rsa-sha256; q=dns/txt; c=relaxed/relaxed;
 ...
 bh=94ZIr2Ld0CngPdLlYLNzf7kqIHa4T7UaLim32xyHk2A=; b=gPzBrwtLMpNs1esnlPxCBORp+H
 tnAciF1joT4Cvf0dIb6UYerwiopn7ivnTu0zc87cUzlZWXqyTjYKL2hcHMHU0+mp5gVcGyy85DPEe
 b8oAaBl170JXru9AIShwld99vd9IK97IHkn96eBt4DXjM6zsCfARjr1pkG/AP0cmG1XU=;
DKIM-Signature: v=1; a=rsa-sha256; q=dns/txt; c=relaxed/relaxed; d=sf.net; s=x
 ...
 bh=94ZIr2Ld0CngPdLlYLNzf7kqIHa4T7UaLim32xyHk2A=; b=PcMZxLCriGgIJHLP2GnvUCdhPc
 Q2y66cnSX8KbR0X5Yaij8AeUQt4IPzu2v9YYZyK1IdXztQW/lwWJIFq+imDeQ8F2651JdLIzyfLWM
 E9NLIGR+bw34s6hIRfv7nzyVV3tNrz+hwAR37eEkKspxym60S1i0gOZkvkakxtWJYPbc=;
...
Received: from localhost (unknown [104.132.0.81])
 ...
 by mail.kernel.org (Postfix) with ESMTPSA id 26B7820873;
 Thu, 31 Oct 2019 16:01:06 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
 s=default; t=1572537666;
 bh=CZzQvxKLjEjSKj3O2Elt79ic/rBXkNVOpYK7Yf7A3HA=;
 h=Date:From:To:Cc:Subject:References:In-Reply-To:From;
 b=e8MpJYNTUyr78LEONls9hun31YqgvGD5oQTWLzQs+yrwP//XWO5ufrIWB8i6oz9dH
 OIlDjHaMIUdg07ImE6yOt7s7ycEM8v8hoS25BZHet0bQzaRrOYitfkds2urnpUMZod
 B5MnBXlKLnAZJUfcyoVWraG1+tq6TP+9jJBdvENU=
Date: Thu, 31 Oct 2019 09:01:05 -0700
From: Jaegeuk Kim <jaegeuk@kernel.org>
To: Chao Yu <yuchao0@huawei.com>
Message-ID: <20191031160105.GC60005@jaegeuk-macbookpro.roam.corp.google.com>
References: <20191029092730.109428-1-yuchao0@huawei.com>
MIME-Version: 1.0
Content-Disposition: inline
In-Reply-To: <20191029092730.109428-1-yuchao0@huawei.com>
...
X-Headers-End: 1iQCsu-00Cg5d-Tk
...
> @@ -379,7 +414,6 @@ static bool __has_merged_page(struct bio *bio, struct inode *inode,
...
>  
>  	if (!bio)
> @@ -389,10 +423,12 @@ static bool __has_merged_page(struct bio *bio, struct inode *inode,
>  		return true;
>  
...
>  	fio->submitted = true;
> @@ -796,7 +837,8 @@ static struct bio *f2fs_grab_read_bio(struct inode *inode, block_t blkaddr,
>  
...
>  
> @@ -807,6 +849,7 @@ static struct bio *f2fs_grab_read_bio(struct inode *inode, block_t blkaddr,
>  			return ERR_PTR(-ENOMEM);
>  		}
>  		ctx->bio = bio;
> +		ctx->sbi = sbi;
...
>  		bio->bi_private = ctx;
>  	}
> @@ -1871,6 +1914,144 @@ static int f2fs_read_single_page(struct inode *inode, struct page *page,
>  	return ret;
>  }
>  
> +#ifdef F2FS_FS_COMPRESSION
...
> @@ -1880,7 +2061,7 @@ static int f2fs_read_single_page(struct inode *inode, struct page *page,
...
>  {
> @@ -1888,6 +2069,19 @@ static int f2fs_mpage_readpages(struct address_space *mapping,
...
>  
>  	map.m_pblk = 0;
> @@ -1911,9 +2105,40 @@ static int f2fs_mpage_readpages(struct address_space *mapping,
>  				goto next_page;
>  		}
>  
...
>  			zero_user_segment(page, 0, PAGE_SIZE);
>  			unlock_page(page);
> @@ -1921,6 +2146,17 @@ static int f2fs_mpage_readpages(struct address_space *mapping,
>  next_page:
>  		if (pages)
>  			put_page(page);
> +
...
>  	if (bio)
> @@ -1935,6 +2171,11 @@ static int f2fs_read_data_page(struct file *file, struct page *page)
>  
...
>  				sector_t *last_block,
>  				struct writeback_control *wbc,
> @@ -2201,7 +2446,7 @@ static int __write_data_page(struct page *page, bool *submitted,
...
>  	unsigned offset = 0;
> @@ -2330,7 +2575,7 @@ static int __write_data_page(struct page *page, bool *submitted,
>  	}
>  
>  	if (submitted)
...
>  	return 0;
>  
> @@ -2351,7 +2596,8 @@ static int __write_data_page(struct page *page, bool *submitted,
...
>  
>  /*
> @@ -2369,6 +2615,22 @@ static int f2fs_write_cache_pages(struct address_space *mapping,
...
>  	pgoff_t index;
> @@ -2378,6 +2640,8 @@ static int f2fs_write_cache_pages(struct address_space *mapping,
>  	int range_whole = 0;
>  	xa_mark_t tag;
>  	int nwritten = 0;
> +	int submitted = 0;
...
>  	pagevec_init(&pvec);
>  
> @@ -2411,8 +2675,6 @@ static int f2fs_write_cache_pages(struct address_space *mapping,
...
>  				tag);
>  		if (nr_pages == 0)
> @@ -2420,7 +2682,26 @@ static int f2fs_write_cache_pages(struct address_space *mapping,
>  
...
> @@ -2430,7 +2711,7 @@ static int f2fs_write_cache_pages(struct address_space *mapping,
>  			}
>  
...
> @@ -2455,44 +2736,63 @@ static int f2fs_write_cache_pages(struct address_space *mapping,
...
>  		cycled = 1;
>  		index = 0;
> @@ -2509,6 +2809,9 @@ static int f2fs_write_cache_pages(struct address_space *mapping,
>  	if (bio)
...
>  }
>  
> @@ -2517,6 +2820,8 @@ static inline bool __should_serialize_io(struct inode *inode,
>  {
...
> @@ -2749,6 +3055,33 @@ static int f2fs_write_begin(struct file *file, struct address_space *mapping,
>  		if (err)
>  			goto fail;
>  	}
> +
...
> @@ -2761,6 +3094,8 @@ static int f2fs_write_begin(struct file *file, struct address_space *mapping,
>  		goto fail;
>  	}
>  
...
> @@ -2375,11 +2477,13 @@ static inline void f2fs_change_bit(unsigned int nr, char *addr)
>  /*
...
> @@ -2388,7 +2492,7 @@ static inline void f2fs_change_bit(unsigned int nr, char *addr)
...
> @@ -2456,6 +2561,7 @@ static inline void __mark_inode_dirty_flag(struct inode *inode,
>  	case FI_DATA_EXIST:
>  	case FI_INLINE_DOTS:
>  	case FI_PIN_FILE:
...
>  	}
>  }
> @@ -2611,16 +2717,39 @@ static inline int f2fs_has_inline_xattr(struct inode *inode)
...
> @@ -2780,7 +2909,8 @@ static inline bool f2fs_may_extent_tree(struct inode *inode)
>  	struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
>  
...
>  
>  	/*
> @@ -2900,7 +3030,8 @@ static inline void verify_blkaddr(struct f2fs_sb_info *sbi,
>  
...
> @@ -3239,8 +3373,13 @@ int f2fs_map_blocks(struct inode *inode, struct f2fs_map_blocks *map,
>  			int create, int flag);
...
> @@ -3250,6 +3389,10 @@ int f2fs_migrate_page(struct address_space *mapping, struct page *newpage,
>  #endif
...
> @@ -3366,6 +3510,20 @@ static inline struct f2fs_stat_info *F2FS_STAT(struct f2fs_sb_info *sbi)
...
> @@ -3599,9 +3761,52 @@ static inline void f2fs_set_encrypted_inode(struct inode *inode)
>   */
...
> +static inline bool f2fs_is_compress_backend_ready(struct inode *inode) { return true; }
...
> @@ -3701,6 +3907,15 @@ static inline bool f2fs_may_encrypt(struct inode *inode)
>  #endif
>  }
>  
...
> --- a/fs/f2fs/file.c
> +++ b/fs/f2fs/file.c
> @@ -485,6 +485,9 @@ static int f2fs_file_mmap(struct file *file, struct vm_area_struct *vma)
...
>  	if (err)
> @@ -518,6 +521,9 @@ void f2fs_truncate_data_blocks_range(struct dnode_of_data *dn, int count)
...
> @@ -525,26 +531,51 @@ void f2fs_truncate_data_blocks_range(struct dnode_of_data *dn, int count)
...
>  		pgoff_t fofs;
>  		/*
> @@ -587,6 +618,9 @@ static int truncate_partial_data_page(struct inode *inode, u64 from,
>  		return 0;
>  	}
>  
> +	if (f2fs_compressed_file(inode))
...
> @@ -602,7 +636,7 @@ static int truncate_partial_data_page(struct inode *inode, u64 from,
>  	return 0;
>  }
>  
...
>  	struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
>  	struct dnode_of_data dn;
> @@ -667,6 +701,24 @@ int f2fs_truncate_blocks(struct inode *inode, u64 from, bool lock)
>  	return err;
>  }
>  
> +int f2fs_truncate_blocks(struct inode *inode, u64 from, bool lock)
...
>  {
>  	int err;
> @@ -783,6 +835,10 @@ int f2fs_setattr(struct dentry *dentry, struct iattr *attr)
>  	if (unlikely(f2fs_cp_error(F2FS_I_SB(inode))))
>  		return -EIO;
>  
...
>  	if (err)
>  		return err;
> @@ -1023,8 +1079,8 @@ static int __read_out_blkaddrs(struct inode *inode, block_t *blkaddr,
>  	} else if (ret == -ENOENT) {
>  		if (dn.max_level == 0)
>  			return -ENOENT;
...
> @@ -1713,7 +1776,45 @@ static int f2fs_setflags_common(struct inode *inode, u32 iflags, u32 mask)
>  			return -ENOTEMPTY;
>  	}
>  
...
>  		FS_IMMUTABLE_FL |	\
>  		FS_APPEND_FL |		\
> @@ -3091,6 +3198,17 @@ static int f2fs_ioc_set_pin_file(struct file *filp, unsigned long arg)
>  		ret = -EAGAIN;
>  		goto out;
>  	}
> +
...
>  	if (ret)
>  		goto out;
> @@ -3343,6 +3461,17 @@ long f2fs_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
>  	}
>  }
>  
...
>  {
>  	struct file *file = iocb->ki_filp;
> @@ -3354,6 +3483,9 @@ static ssize_t f2fs_file_write_iter(struct kiocb *iocb, struct iov_iter *from)
>  		goto out;
>  	}
>  
> +	if (!f2fs_is_compress_backend_ready(inode))
...
>  			ret = -EAGAIN;
> @@ -3467,7 +3599,7 @@ long f2fs_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
>  
...
> --- a/fs/f2fs/namei.c
> +++ b/fs/f2fs/namei.c
> @@ -119,6 +119,16 @@ static struct inode *f2fs_new_inode(struct inode *dir, umode_t mode)
...
> @@ -262,6 +275,35 @@ int f2fs_update_extension_list(struct f2fs_sb_info *sbi, const char *name,
>  	return 0;
>  }
>  
...
>  						bool excl)
>  {
> @@ -286,6 +328,8 @@ static int f2fs_create(struct inode *dir, struct dentry *dentry, umode_t mode,
...
> @@ -297,6 +341,7 @@ static int f2fs_create(struct inode *dir, struct dentry *dentry, umode_t mode,
>  		goto out;
>  	f2fs_unlock_op(sbi);
>  
> +	stat_inc_compr_inode(inode);
