...
Received: from lists.sourceforge.net (lists.sourceforge.net [216.105.38.7])
	by mail.lfdr.de (Postfix) with ESMTPS id D376AFB121
	for <lists+linux-f2fs-devel@lfdr.de>; Wed, 13 Nov 2019 14:13:47 +0100 (CET)
Received: from [127.0.0.1] (helo=sfs-ml-1.v29.lw.sourceforge.com)
	by sfs-ml-1.v29.lw.sourceforge.com with esmtp (Exim 4.90_1)
	(envelope-from <linux-f2fs-devel-bounces@lists.sourceforge.net>)
	id 1iUsSw-00087e-LQ; Wed, 13 Nov 2019 13:13:46 +0000
Received: from [172.30.20.202] (helo=mx.sourceforge.net)
 by sfs-ml-1.v29.lw.sourceforge.com with esmtps
 (TLSv1.2:ECDHE-RSA-AES256-GCM-SHA384:256) (Exim 4.90_1)
 (envelope-from <yuchao0@huawei.com>) id 1iUsSv-00087N-DO
 for linux-f2fs-devel@lists.sourceforge.net; Wed, 13 Nov 2019 13:13:45 +0000
DKIM-Signature: v=1; a=rsa-sha256; q=dns/txt; c=relaxed/relaxed;
 ...
 bh=AEdKo9sS9dcNTXQczNdxPLjdPADxEfFLJAl9SqE+Mb8=; b=WtUM3T+T3mxnIgbidziu4oeukd
 Cn2MioeGUJ8gTS6VAV/KFerkVDi/1/eUWlQFfGckxBK46LoXfRWP1s6GPAEDA6JhzKZOsqKXspP/n
 PHKHQWc7J2pL4NoCcAj6KV28IxaEckTHmuT2pIPIbMI+Vi8GAsh0WzyGaiaRNtyDEpUY=;
DKIM-Signature: v=1; a=rsa-sha256; q=dns/txt; c=relaxed/relaxed; d=sf.net; s=x
 ...
 List-Owner:List-Archive; bh=AEdKo9sS9dcNTXQczNdxPLjdPADxEfFLJAl9SqE+Mb8=; b=N
 oiOSraveIrzw7+VV1U8RP4YvyW4DfODtWjtgyVQjR65KOKtDV0adYitLjYb+GSyk8SN4VTnh7Hggv
 9A05PObqcztwJPtzN00zRj+Zlgizm+JONcq7g1w6CsGZxw59XJS2sFyxz+lCbCRF2gEQ4ErDS+Ap+
 AVITPazvBJ+HY0UY=;
...
Received: from DGGEMS402-HUB.china.huawei.com (unknown [172.30.72.60])
 by Forcepoint Email with ESMTP id DB43F23241F79083D1A6;
 Wed, 13 Nov 2019 21:13:31 +0800 (CST)
Received: from szvp000203569.huawei.com (10.120.216.130) by
 DGGEMS402-HUB.china.huawei.com (10.3.19.202) with Microsoft SMTP Server id
 14.3.439.0; Wed, 13 Nov 2019 21:13:25 +0800
From: Chao Yu <yuchao0@huawei.com>
To: <jaegeuk@kernel.org>
Date: Wed, 13 Nov 2019 21:13:10 +0800
Message-ID: <20191113131310.17568-1-yuchao0@huawei.com>
...
X-Headers-End: 1iUsSp-00F5bs-1N
Subject: [f2fs-dev] [RFC PATCH v4] f2fs: support data compression
...
index 000000000000..254275325890
--- /dev/null
...
index 9a2e1b078115..2d64c6ffee84 100644
--- a/fs/f2fs/data.c
+++ b/fs/f2fs/data.c
...
+
 ...
 static int f2fs_submit_page_read(struct inode *inode, struct page *page,
 							block_t blkaddr)
@@ -1872,6 +1973,145 @@ static int f2fs_read_single_page(struct inode *inode, struct page *page,
 	return ret;
 }
...
@@ -1881,7 +2121,7 @@ static int f2fs_read_single_page(struct inode *inode, struct page *page,
...
+int f2fs_mpage_readpages(struct address_space *mapping,
 ...
 {
@@ -1889,6 +2129,19 @@ static int f2fs_mpage_readpages(struct address_space *mapping,
...
+	unsigned max_nr_pages = nr_pages;
 	...
 	map.m_pblk = 0;
@@ -1912,9 +2165,40 @@ static int f2fs_mpage_readpages(struct address_space *mapping,
 				goto next_page;
 		}
...
+#endif
 			...
 			zero_user_segment(page, 0, PAGE_SIZE);
 			unlock_page(page);
@@ -1922,6 +2206,17 @@ static int f2fs_mpage_readpages(struct address_space *mapping,
 next_page:
 		if (pages)
 			put_page(page);
+
...
+#endif
 	...
 	if (bio)
@@ -1936,6 +2231,11 @@ static int f2fs_read_data_page(struct file *file, struct page *page)
...
@@ -1954,6 +2254,9 @@ static int f2fs_read_data_pages(struct file *file,
...
+
 	...
 	if (f2fs_has_inline_data(inode))
 		return 0;
@@ -1961,22 +2264,23 @@ static int f2fs_read_data_pages(struct file *file,
...
@@ -2136,7 +2440,7 @@ int f2fs_do_write_data_page(struct f2fs_io_info *fio)
 	if (ipu_force ||
...
@@ -2172,7 +2476,7 @@ int f2fs_do_write_data_page(struct f2fs_io_info *fio)
...
@@ -2193,7 +2497,7 @@ int f2fs_do_write_data_page(struct f2fs_io_info *fio)
 	return err;
 }
...
+int f2fs_write_single_data_page(struct page *page, int *submitted,
 				...
 				sector_t *last_block,
 				struct writeback_control *wbc,
@@ -2202,7 +2506,7 @@ static int __write_data_page(struct page *page, bool *submitted,
...
+	const pgoff_t end_index = ((unsigned long long)i_size)
 	...
 	unsigned nr_pages = DIV_ROUND_UP(i_size, PAGE_SIZE);
@@ -2242,7 +2546,8 @@ static int __write_data_page(struct page *page, bool *submitted,
 	if (unlikely(is_sbi_flag_set(sbi, SBI_POR_DOING)))
 		goto redirty_out;
...
+		page->index < end_index || f2fs_verity_in_progress(inode))
 	...
 	/*
@@ -2317,7 +2622,6 @@ static int __write_data_page(struct page *page, bool *submitted,
...
@@ -2330,7 +2634,7 @@ static int __write_data_page(struct page *page, bool *submitted,
 	}
...
@@ -2351,7 +2655,19 @@ static int __write_data_page(struct page *page, bool *submitted,
...
+						wbc, FS_DATA_IO);
 ...
 /*
@@ -2364,11 +2680,27 @@ static int f2fs_write_cache_pages(struct address_space *mapping,
 					enum iostat_type io_type)
 {
 	int ret = 0;
-	int done = 0;
...
+#endif
 	...
 	pgoff_t index;
@@ -2378,6 +2710,8 @@ static int f2fs_write_cache_pages(struct address_space *mapping,
 	int range_whole = 0;
 	xa_mark_t tag;
 	int nwritten = 0;
+	int submitted = 0;
...
@@ -2407,12 +2741,11 @@ static int f2fs_write_cache_pages(struct address_space *mapping,
 	else
...
+	while (!done && !retry && (index <= end)) {
 		nr_pages = pagevec_lookup_range_tag(&pvec, mapping, &index, end,
 				tag);
 		if (nr_pages == 0)
@@ -2420,15 +2753,61 @@ static int f2fs_write_cache_pages(struct address_space *mapping,
...
+#endif
 ...
 retry_write:
 			lock_page(page);
@@ -2455,45 +2834,70 @@ static int f2fs_write_cache_pages(struct address_space *mapping,
...
@@ -2517,6 +2921,8 @@ static inline bool __should_serialize_io(struct inode *inode,
 {
...
@@ -2659,6 +3065,7 @@ static int prepare_write_begin(struct f2fs_sb_info *sbi,
...
@@ -2749,6 +3156,24 @@ static int f2fs_write_begin(struct file *file, struct address_space *mapping,
 		if (err)
 			goto fail;
 	}
+
...
@@ -2761,6 +3186,8 @@ static int f2fs_write_begin(struct file *file, struct address_space *mapping,
 		goto fail;
 	}
...
@@ -2844,6 +3271,16 @@ static int f2fs_write_end(struct file *file,
 		else
 			SetPageUptodate(page);
 	}
+
...
@@ -3234,6 +3671,15 @@ static int f2fs_swap_activate(struct swap_info_struct *sis, struct file *file,
 	if (ret)
 		return ret;
...
+
 	...
 	if (ret)
 		return ret;
@@ -3318,6 +3764,27 @@ void f2fs_destroy_post_read_processing(void)
...
index 77e04f252a04..20067fa3b035 100644
--- a/fs/f2fs/f2fs.h
+++ b/fs/f2fs/f2fs.h
...
@@ -3596,8 +3764,57 @@ static inline void f2fs_set_encrypted_inode(struct inode *inode)
  */
...
@@ -3617,6 +3834,7 @@ F2FS_FEATURE_FUNCS(lost_found, LOST_FOUND);
...
@@ -3698,6 +3916,15 @@ static inline bool f2fs_may_encrypt(struct inode *inode)
 #endif
 }
...
+
 ...
 {
@@ -3729,6 +3956,8 @@ static inline bool f2fs_force_buffered_io(struct inode *inode,
 		return true;
